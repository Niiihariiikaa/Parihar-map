...
export default function RestroomFinder() {
  ...
  const searchedLocationRef = useRef<Location | null>(null);
  ...
  const getRoute = async (start: Location, end: Location) => {
    ...
    map.addLayer({
      id: routeLayerId, type: "line", source: routeLayerId,
  ...
  const findPlaces = async () => {
    if (!searchedLocationRef.current) { alert("Search a location first"); return; }
    ...
    (data.elements as OverpassElement[]).forEach((place) => {
      ...
      const popup = new maplibregl.Popup().setHTML(`
        ...
        ${isPublic ? "Public Restroom" : "Restroom Likely"}<br/><br/>
        <button style="padding:6px 10px;border-radius:6px;border:none;background:#1a73e8;color:white;cursor:pointer;">
          Get Directions
        </button>
      `);
      ...
      popup.on("open", () => {
        ...
        if (btn) {
          btn.onclick = async () => {
            if (!searchedLocationRef.current) return;
    ...
    <div style={{ width: "100vw", height: "100vh" }}>
      <div style={{ position: "absolute", top: 20, left: "50%", transform: "translateX(-50%)", zIndex: 10 }}>
        <input
          ...
          style={{ padding: "10px 16px", borderRadius: 20, width: 280, fontSize: 16 }}
        />
        {suggestions.length > 0 && (
          <div style={{ background: "white", borderRadius: 12, marginTop: 6, maxHeight: 200, overflowY: "auto", boxShadow: "0 4px 10px rgba(0,0,0,0.15)" }}>
            {suggestions.map((place, i) => (
              <div key={i} onClick={() => selectLocation(place)} style={{ padding: "8px 12px", cursor: "pointer", fontSize: 13 }}>
                {place.display_name}
              </div>
            ))}
          </div>
      ...
      <button onClick={findPlaces} style={{ position: "absolute", top: 80, left: "50%", transform: "translateX(-50%)", zIndex: 10, padding: "8px 14px", borderRadius: 20, background: "black", color: "white", cursor: "pointer" }}>
